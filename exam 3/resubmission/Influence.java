...
public class Influence {
    ...
    public Influence(SimplifiedDigraph givenGraph) {
        ...
        for (int i = 0; i < givenGraph.V(); i++) {
            if (givenGraph.outdegree(i) > 0) {
                sources.enqueue(i);
                // StdOut.print(i + " ");
            }
        }
        bfs(givenGraph, sources);
    ...
    private void computeNodeInfluence(SimplifiedDigraph givenGraph, int fromIndex) {
        Iterable<Integer> adj = givenGraph.adj(fromIndex);
        if (givenGraph.outdegree(fromIndex) == 0 && givenGraph.indegree(fromIndex) != 0) {
            for (int i = 0; i < givenGraph.V(); i++) {
                if (fromIndex != i) {
                    givenGraph.addEdge(fromIndex, i);
                }
            }
        }

        for (int a : adj) {
            Iterable<Integer> nextadj = givenGraph.adj(a);
            // givenGraph.addEdge(fromIndex, a);
            // StdOut.print("a " + a + " ");
            for (int b : nextadj) {
                if (check(adj, b) && b != fromIndex
                        && givenGraph.outdegree(fromIndex) < (givenGraph
                        .V() - 1)) {
                    givenGraph.addEdge(fromIndex, b);
                    Iterable<Integer> badj = givenGraph.adj(b);
                    if (check(badj, fromIndex))
                        givenGraph.addEdge(b, fromIndex);
                }
            }
        }
    ...
    // checks if a vertex is already in the adj list
    private boolean check(Iterable<Integer> adj, int vertex) {
        for (int i : adj) {
            if (vertex == i)
                return false;
    ...
    // bfs to begin for influencegraph
    private void bfs(SimplifiedDigraph graph, Iterable<Integer> sources) {
